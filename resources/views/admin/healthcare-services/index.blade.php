...
@section('heading', 'Healthcare Services')

@section('content')
    <x-admin.page-header title="Healthcare Services" subtitle="Services shown in “Healthcare Services Around You”.">
        <x-slot:action>
            <a href="{{ route('admin.healthcare-services.create') }}" class="inline-flex h-10 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground hover:bg-primary/90">
    ...
    <x-admin.card padding="p-0">
        <div class="p-5 pb-0">
            <x-admin.filter-bar :action="route('admin.healthcare-services.index')" :search="$search" placeholder="Search services..."
                :statuses="['' => 'All Status', '1' => 'Active', '0' => 'Inactive']" :status="$status" />
        </div>

        @if ($services->isEmpty())
            <p class="px-5 py-12 text-center text-sm text-muted-foreground">No services found.</p>
        @else
                    ...
                    <tbody class="divide-y divide-border">
                        @foreach ($services as $service)
                            <tr class="hover:bg-muted/30">
                                ...
                                <td class="px-5 py-3 text-muted-foreground">{{ $service->sort_order }}</td>
                                <td class="px-5 py-3"><x-admin.status-badge :status="(bool) $service->is_active" /></td>
                                <td class="px-5 py-3">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.healthcare-services.edit', $service) }}" class="rounded-md p-2 text-muted-foreground hover:bg-muted hover:text-primary">
                                        ...
                                        <form method="POST" action="{{ route('admin.healthcare-services.destroy', $service) }}" onsubmit="return confirm('Delete this service?')">
                                            @csrf @method('DELETE')
            ...
            <div class="border-t border-border p-4">{{ $services->links() }}</div>
        @endif
    </x-admin.card>
@endsection
